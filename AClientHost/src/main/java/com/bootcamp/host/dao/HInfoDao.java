...
System.out.println("imageUpdate dao");
	...
	public Boolean arHost(String scId) {
		...
		try {
			...
			if (resultSet.next()) {
				...
				System.out.println("count Daoid" + scId);
			}
	...
	public int arHost1(String shId) {

		System.out.println("hostClientDto arrrrrrr");
		Connection connection = null;
		int hSeq = 0;
		...
		try {
			connection = dataSource.getConnection();
			String query = "select hSeq from client, host where cId=hId and cId=? ";
			...
			if (resultSet.next()) {
				// 하나니까 if로.
				hSeq = resultSet.getInt("hSeq");

			}
			System.out.println("hinfodao hseq" + hSeq);
